...
if (_simu == "soldier") then
{
	...
	( _grp) setVariable ["GAIA_ZONE_INTEND",[(missionNamespace getVariable ["mcc_zone_markername","0"]), (["MOVE","NOFOLLOW","FORTIFY"]call BIS_fnc_selectRandom)], true];
...
else
{
	...
	if (count(_unitarray)>0) then
	{
		...
		( _grp) setVariable ["GAIA_ZONE_INTEND",[(missionNamespace getVariable ["mcc_zone_markername","0"]), (["MOVE","NOFOLLOW"]call BIS_fnc_selectRandom)], true];
	};
};
